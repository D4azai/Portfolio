...
Owner-supplied changes: AYNKO is now presented as the practice of two co-founders, Aymane Chellak and Zakaria Bak, with both contacts in About, Contact, the enquiry dialog, the footer, and the schema.org founders list. A LinkedIn profile link is published; a GitHub slot exists in `src/data/portfolio.js` and stays hidden until a URL is supplied.

Conversion and legibility: the selected-work showcase is visible without a disclosure click; the smallest mono labels were raised from 5–9px to a 10px floor (11px base); the intro plays once per session (`sessionStorage`) and is bypassed for deep links, with the footer replay still available. The hero's WebGL scene is created only after the intro closes, so a single renderer exists at any time. The build removes stale esbuild chunks and unreferenced legacy stylesheets and scripts from `dist`. Link previews use a branded `assets/og.jpg` generated by `tools/og-image.mjs`.

Still deliberately absent: project outcomes, metrics, and testimonials. See CONTENT-NEEDED.md for the approved-evidence requirements before adding them.
